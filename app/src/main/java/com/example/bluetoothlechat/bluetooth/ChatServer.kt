...
object ChatServer {
    ...
    var connectedDeviceMap : HashMap<String, BluetoothDevice> = HashMap<String, BluetoothDevice> ()
    var characteristicMap : HashMap<String, BluetoothGattCharacteristic> = HashMap<String, BluetoothGattCharacteristic> ()
    ...
    /**fun setCurrentChatConnection(device: BluetoothDevice) {
        Log.d(TAG, "Setting connection to " + device.address)
    ...
    fun setChatConnection(device: BluetoothDevice) {
        Log.d(TAG, "Setting connection to " + device.address)
        _deviceConnection.value = DeviceConnectionState.Connected(device)
        connectToChatDevice(device)
    }
    ...
    fun sendMessage(message: String): Boolean {
        Log.d(TAG, connectedDeviceMap.toString())
        Log.d(TAG, connectedGattMap.toString())
        Log.d(TAG, characteristicMap.toString())
        for ((key, value) in connectedDeviceMap) {
            var device: BluetoothDevice = value
            var characteristic: BluetoothGattCharacteristic? = characteristicMap.get(device.address)
            Log.d(TAG, "Send a message to " + device.address)
            characteristic?.let { characteristic ->
                characteristic.writeType = BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE

                val messageBytes = message.toByteArray(Charsets.UTF_8)
                characteristic.value = messageBytes
                var gatt: BluetoothGatt? = connectedGattMap.get(device.address)
                gatt?.let {
    ...
    private fun setupGattService(): BluetoothGattService {
        // Setup gatt service
        val service = BluetoothGattService(UART_UUID, BluetoothGattService.SERVICE_TYPE_PRIMARY)
    ...
    private class GattClientCallback : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (isSuccess && isConnected) {
                if (!connectedGattMap.containsKey(gatt.device.address)) {
                    connectedGattMap.put(gatt.device.address, gatt)
                }
                // discover services
                gatt.discoverServices()
            } else if (isDisconnected){
                if (connectedGattMap.containsKey(gatt.device.address)) {
                    var bluetoothGatt: BluetoothGatt? = connectedGattMap.get(gatt.device.address)
                    if( bluetoothGatt != null ){
                        bluetoothGatt.close()
                        bluetoothGatt = null
                    }
                    connectedGattMap.remove(gatt.device.address)
                }
            }
        }
